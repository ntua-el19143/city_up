...
class _HomePageState extends State<_HomePage>{
  ...
  Future<Position> _getCurrentLocation() async{
    ...
    if (permission == LocationPermission.deniedForever){
      return Future.error('Location Permission Denied Forever. Cannot request!');
    }
  ...
  int _counter=0;
   void _incrementCounter() {  // Θελω να καλω αυτη τη συναρτηση οταν παταω POST για story ειτε για article
    setState(() {
      _counter++;
    });
  }
  ...
  Widget build(BuildContext context) {
   return Scaffold(
    appBar: AppBar(
      ...
      title: const Text('City Up', style: TextStyle(color:Colors.lightBlue, fontStyle: FontStyle.italic, fontWeight: FontWeight.w900, fontSize: 25),
      ),
      actions: [
        IconButton(
         onPressed: () {},
      ...
      children: <Widget>[ 
        ...
        Form(
          child: Row(
            ...
            builder: (context)=> LifestyleOptions()))}, child: Text('LIFESTYLE', style: TextStyle(color:Colors.white, fontSize: 20.0)), style: ButtonStyle(backgroundColor: MaterialStateProperty.all(Colors.lightBlue)),),
              TextButton(onPressed: ()=> {Navigator.of(context).push(MaterialPageRoute(
            builder: (context)=> NewsOptions()))}
              , child: Text('NEWS', style: TextStyle(color:Colors.white, fontSize: 20.0)), style: ButtonStyle(backgroundColor: MaterialStateProperty.all(Colors.lightBlue)),),
              TextButton(onPressed: ()=> {Navigator.of(context).push(MaterialPageRoute(
            builder: (context)=> CityInfoOptions()))}, child: Text('CITY INFO', style: TextStyle(color:Colors.white, fontSize: 20.0)), style: ButtonStyle(backgroundColor: MaterialStateProperty.all(Colors.lightBlue)),),

      ],) ),),
      const SizedBox(height:8),
      ElevatedButton(onPressed: () {_getCurrentLocation().then((value) { lat = '${value.latitude}'; long = '${value.longitude}';});
      }, 
